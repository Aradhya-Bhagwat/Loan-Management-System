...
struct AdminProfileView: View {
    ...
    private var adminName: String { authController.currentUser?.name ?? "Admin User" }
    private var adminRole: String { authController.currentUser?.role.title ?? "Admin" }

    private var employeeId: String {
        guard let id = authController.currentUser?.id.uuidString.prefix(8) else {
            return "ADM-000001"
        }
        return "ADM-\(id.uppercased())"
    }

    private var departmentText: String {
        authController.currentUser?.department ?? "Loan Operations"
    }
    ...
    var body: some View {
        Form {
            Section {
                ProfileAvatarHeader(
                    name: authController.currentUser?.name ?? "Admin User",
            ...
            Section("Security") {
                ...
                SettingsReadOnlyRow(title: "Branch", value: selectedBranch.title)
                SettingsReadOnlyRow(title: "Role", value: adminRole)
            }
        ...
        .sheet(isPresented: $showNameEditor) {
            ProfileTextEditSheet(
                ...
                saveProfile(name: newName, email: editableEmail, phone: editablePhone, branch: selectedBranch)
            }
        }
        .sheet(isPresented: $showPhoneEditor) {
            ProfileTextEditSheet(
                ...
                saveProfile(name: editableName, email: editableEmail, phone: newPhone, branch: selectedBranch)
            }
        }
        .sheet(isPresented: $showEmailEditor) {
            ProfileTextEditSheet(
                ...
                saveProfile(name: editableName, email: newEmail, phone: editablePhone, branch: selectedBranch)
            }
        }
        .onAppear {
            syncEditableFields()
    ...
    private func syncEditableFields() {
        ...
        selectedBranch = Branch(rawValue: user.branch ?? "") ?? .north
    }

    private func saveProfile(name: String, email: String, phone: String, branch: Branch) {
        guard let user = authController.currentUser else { return }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    selectedBranch = Branch(rawValue: refreshedSession.branch ?? "") ?? .north
                    isSaving = false
                    showSuccessAlert(
                        message:
